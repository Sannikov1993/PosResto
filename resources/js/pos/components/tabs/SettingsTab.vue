...
const saveSettings = async () => {
    ...
    try {
        ...
        localStorage.setItem('poslab_settings', JSON.stringify(settings));
        localStorage.setItem('poslab_settings_saved', new Date().toISOString());
...
const loadSettings = async () => {
    ...
    try {
        ...
        const savedTime = localStorage.getItem('poslab_settings_saved');
        if (savedTime) {
        ...
        try {
            const localSettings = localStorage.getItem('poslab_settings');
            if (localSettings) {
...
watch(settings, () => {
    ...
    saveTimeout = setTimeout(() => {
        localStorage.setItem('poslab_settings_draft', JSON.stringify(settings));
    }, 1000);
}, { deep: true });
</script>
