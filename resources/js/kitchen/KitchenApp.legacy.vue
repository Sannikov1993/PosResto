<template>
    <div class="min-h-screen bg-gray-900 text-white">
        ...
        <!-- Not Linked State - Enter Linking Code -->
        <div v-else-if="deviceStatus === 'not_linked'" class="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-900 to-gray-800">
            <div class="text-center max-w-lg">
                <div class="text-8xl mb-6">🔑</div>
                ...
                <button
                    ...
                    :disabled="linkingCodeDigits.join('').length !== 6 || linkingLoading"
                ...
                >
                    <span v-if="linkingLoading" class="animate-spin">⏳</span>
                    <span>{{ linkingLoading ? 'Привязка...' : 'Привязать' }}</span>
                </button>
...
// Get auth headers if backoffice token is available
const getAuthHeaders = () => {
    const token = localStorage.getItem('backoffice_token') || localStorage.getItem('pos_token');
    if (token) {
...
// Submit linking code
const submitLinkingCode = async () => {
    const code = linkingCodeDigits.value.join('');
    ...
    try {
        const res = await axios.post('/api/kitchen-devices/link', {
        ...
        if (e.response?.data?.error_code === 'invalid_code') {
            ...
            linkingError.value = e.response?.data?.message || 'Ошибка привязки';
        }
        // Clear code
...
const loadOrderCounts = async () => {
    try {
        ...
        let url = `/api/kitchen-devices/orders/count-by-dates?device_id=${deviceId.value}&start_date=${startDate}&end_date=${endDate}`;
...
const fetchOrders = async () => {
    try {
        // Используем специальный endpoint для устройств кухни (не требует авторизации)
        let url = `/api/kitchen-devices/orders?device_id=${deviceId.value}&date=${selectedDate.value}`;
        if (stationSlug.value) {
            url += `&station=${stationSlug.value}`;
        }
        const res = await axios.get(url);
...
const callWaiter = async (order: any) => {
    ...
    try {
        const res = await axios.post(`/api/kitchen-devices/orders/${order.id}/call-waiter`, {
            device_id: deviceId.value
        });
        if (res.data.success) {
